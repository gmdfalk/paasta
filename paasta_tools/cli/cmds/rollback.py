...
def add_subparser(subparsers):
    ...
    list_parser.add_argument(
        ...
        required=False,
        default=None,
    ).completer = lazy_choices_completer(list_previously_deployed_image_versions)
    list_parser.add_argument(
        "-l",
        "--deploy-groups",
...
def list_previously_deployed_image_versions(parsed_args, **kwargs):
    service = parsed_args.service
    soa_dir = parsed_args.soa_dir
    deploy_groups = {
        deploy_group
...
def get_versions_for_service(
    service: str, deploy_groups: List, soa_dir: str
) -> Dict[DeploymentVersion, Tuple]:
...
def list_previous_versions(
    ...
    versions = sorted(versions.items(), key=lambda x: x[1], reverse=True)[:10]
    rows = [("Timestamp -- UTC", "Human time", "deploy_group", "Version")]
    for version, (timestamp, deploy_group) in versions:
        rows.extend(
    ...
    for line in format_table(rows):
        print(line)
    if len(versions) >= 2:
        version, (timestamp, deploy_group) = versions[1]
