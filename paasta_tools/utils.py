...
class DeploymentsJsonV2:
    ...
    ) -> Union[str, DeploymentVersion]:
        # TODO: gate using DeploymentVersion on the actual no-commit redeploy flag (not yet implemented)
        if self.get_image_version_for_deploy_group(deploy_group):
            return DeploymentVersion(
                sha=self.get_git_sha_for_deploy_group(deploy_group),
                image_version=self.get_image_version_for_deploy_group(deploy_group),
            )
        return self.get_git_sha_for_deploy_group(deploy_group)
